...
function inscription (req, res) {
   ...
   function getUserConnected(req, res,next) {
    ...
    jwt.verify(token, config.secret, function(err, decoded) {
      ...
      User.findById(decoded.id, { mot_de_passe: 0 }, function (err, user) {
   ...
   function login(req, res) {
    ...
    User.findOne({ email: req.body.email }, function (err, user) {
      ...
      var passwordIsValid = bcrypt.compareSync(req.body.mot_de_passe, user.mot_de_passe);
  ...
  module.exports = { getUserConnected,inscription,login,logout };